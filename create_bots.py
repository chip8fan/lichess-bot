        ...
        if os.access(file_name, os.X_OK) and "." not in file_name:
            try:
                ...
                if os.path.isdir(binary_name) == False:
                    ...
                    config_lines[33] = config_lines[33].replace("true", "false")
                    config_lines[35] = config_lines[35].replace("true", "false")
                    config_lines[37] = config_lines[37].replace("true", "false")
                    config_lines[60] = config_lines[60].replace("false", "true")
                    config_lines[93] = config_lines[93].replace("  homemade_options:", "# homemade_options:")
                    config_lines[96] = config_lines[96].replace("uci_options:", "# uci_options:")
                    ...
                    config_lines[101] = config_lines[101].replace("UCI_ShowWDL:", "# UCI_ShowWDL:")
                    config_lines[124] = config_lines[124].replace("2000", "60000")
                    config_lines[140] = config_lines[140].replace("20", "180")
                    config_lines[142] = config_lines[142].replace("1800", "10800")
